...
class XlsDataSyncOptionForm(forms.Form):
    ...
    password = forms.CharField(max_length=30,
                               required=False,
                               help_text="Password for XLS protection."
                                         "Note that the workbook will be protected in any case.")
    ...
    def get_xls_template(self, request):
        ...
        if 'apply' in request.POST:
            ...
            if form.is_valid():
                ...
                out = six.BytesIO()
                props = {}
                for e in ('title', 'subject', 'author', 'manager',
                          'company', 'category', 'keywords', 'comments', 'status',):
                    props[e] = form.cleaned_data[e]

                with XlsTemplate(out, {'default_date_format': date_format,
                                       'timezone': timezone},
                                 password=password) as xls:
            ...
            form = XlsDataSyncOptionForm(initial={'filename': filename,
                                                  ...
                                                  'title': '',
                                                  'comments': 'Autogenerated with django-excel-data-sync'},
                                         model=model)
